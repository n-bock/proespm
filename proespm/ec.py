...
class cv(ec):
    ...
    def appendCycle(self, data, id_new, remark):
    ...
    def importBiologic(self, file):
        ...
        self.lines = util.readLines(file, range(53))
        if 'Cyclic Voltammetry' in self.lines[3]:
            self.extract_par = [['self.vs', 'Ei', 'Ei\s\(V\)\s*(\S*)'],
    ...
    def importFile(self, file):
        ...
        if 'EC4' in self._first_line:
            ...
            self.importLabview(file)
        elif 'EC-Lab' in  self._first_line:
            self.importBiologic(file)
...
class peis(ec):
    ...
    def importBiologic(self, file):
        ...
        self.lines = util.readLines(file, range(83))
        if 'Potentio Electrochemical Impedance Spectroscopy' in self.lines[3]:
            self.extract_par = [['self.ecell', 'E (V)', 'E\s\(V\)\s*(\S*)'],
                                ['self.fi', 'fi                  ', '^fi\s*(\S*)'],
...
class chrono(ec):
    ...
    def importBiologic(self, file):
        ...
        self.lines = util.readLines(file, range(57))
        if 'Chrono' in self.lines[3]:
            self.extract_par = [['self.ecell', 'Ei', 'Ei\s\(V\)\s*(\S*)'],
                                ['self.skiprows', 'Nb header lines', 'lines\s:\s*(\d*)\s*']]
